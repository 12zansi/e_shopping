...
class AdminProductImages(UserLogin):
    ...
    def  publish_product_images(self,name: UploadFile, product_id: int, token: str = Depends(token_auth_scheme)):
    ...
    def change_product_images(self, id:int, name: UploadFile, product_id: int, token: str = Depends(token_auth_scheme)):
        user = AdminProductImages._get_user(token)
